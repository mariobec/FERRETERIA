...
**Índice de todos los planes (SD / POS / TEC / LX):** `../00-alineacion/PLAN_INDICE_LHEXIA.md` — fases aquí = prefijo **POS-**.  
**Actualizar este archivo** cada vez que Grok proponga cambios, Cursor implemente, o cambie el criterio de negocio.

**Documentos relacionados:**

- `../03-pos-vendedor/POS_PANTALLA_VENDEDORA_AUDITORIA.md` — mapa técnico para auditar código
- `docs/memory.md` — bitácora breve del proyecto
- `../03-pos-vendedor/POS_REVERT_DOCK_BUSQUEDA.md` — revertir relayout dock + búsqueda alta
...
**Estado Fase 4:** ✅ 2026-05-17 — búsqueda 2+ letras, F8 unificado, toasts error, empty carrito  
**Versión cache POS (Fase 4):** `20260525f`
...
| `../03-pos-vendedor/POS_PANTALLA_VENDEDORA_AUDITORIA.md` | Actualizar si cambia algo contractual |
...
1. ~~Fases 1–4 POS vendedor~~ ✅ (layout `5094d5d` + Fase 4 `20260525f`).
2. **Operativo:** validar búsqueda en piso (filtro Catálogo si Operativo vacío); piloto vale → caja.
3. **Inventario:** toma física — ver `../01-entrega-santo-domingo/CLIENTE_SANTO_DOMINGO.md` (plan entrega, no producto).
4. **QA opcional:** `python scripts/seed_pos_semaforo_pruebas.py` + `pruebas/pos_semaforo/CHECKLIST.md`.
...
| **Revertir** | `../03-pos-vendedor/POS_REVERT_DOCK_BUSQUEDA.md` |
...
| Fase 4 | cache `20260525f` — ver §14 |

---

## 14. Fase 4 — cierre (2026-05-17)

| Ítem | Implementación |
|------|----------------|
| **F2** | Enfoca `#posBuscarManual` (ya existía) |
| **F8** | `posEmitirValeAtajo()` — emite o toast con motivo si botón deshabilitado |
| **F4** | Cotización (ya existía) |
| **Búsqueda** | Mínimo **2** caracteres; mensajes según filtro; `meta.filtrados_por_stock` en API |
| **Errores API** | Sesión expirada / HTTP / JSON inválido en panel |
| **Carrito vacío** | Hint `F2` + `F8` en `premium_cart_cards.html` |
