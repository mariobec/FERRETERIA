...
if ($PublicarEnRepo) {
    $releaseRoot = Join-Path $repo 'release\instalador_tienda'
    Write-Host "Publicando en repo: $releaseRoot" -ForegroundColor Yellow
    if (Test-Path $releaseRoot) { Remove-Item -LiteralPath $releaseRoot -Recurse -Force }
    Copy-Item -LiteralPath $packRoot -Destination $releaseRoot -Recurse -Force
    @"
stamp=$stamp
commit=$commit
branch=$branch
...
dump=$dumpFile
generated=$stamp
"@ | Set-Content (Join-Path $releaseRoot 'MANIFEST.txt') -Encoding UTF8
    Write-Host "Tienda: git pull && instalar_tienda.bat" -ForegroundColor Cyan
}
...
if ($PublicarEnRepo) {
    Write-Host "O desde repo clonado: instalar_tienda.bat (tras git pull)" -ForegroundColor Green
}
